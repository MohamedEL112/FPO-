const defaultInput = 'max = 4x1 + 2x2 \n 3x1 + 2x2 <= 12\nx1 + 2x2 >= 4\n x1 <= 3'
...
const printBFS = () => {
    ...
    const p = createNode('p', [], 'Solution réalisable')
    header.appendChild(p)
    div.appendChild(header)
...
const printTableCard = (phase) => {
    const card = createNode('div', ['card', 'block'])
    const header = createNode('header', ['card-header'])
    const title = createNode('p', ['card-header-title', 'has-background-grey-lighter'], `phase ${phase}`)
    header.appendChild(title)
    card.appendChild(header)
    const contentContainer = createNode('div', ['card-content'])
    const content = createNode('div', ['content', 'overflow-scroll'])
...
const printRatio = (card) => {
    ...
    trBody.forEach((tr, i) => {
        const r = $.ratio[i]
        const txt = isFinite(r) ? `${checkDecimals(r)}` : 'infini'
...
const printAnswer = () => {
    ...
    const iter = createNode('div', ['block'], ``)
